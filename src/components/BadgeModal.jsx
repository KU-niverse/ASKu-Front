...
function BadgeModal({  isOpen, onClose }) {
    ...
    useEffect(()=>{
      const takeMyBadge = async() => {
        try{
          const res= await axios.get(`https://asku.wiki/api/usermypage/badgehistory`, {withCredentials:true})
          if (res.status===200){
            setMyBadge(res.data)
    ...
    const handleRepBadge = async () => {
      try {
        const response = await axios.post(`https://asku.wiki/api/usermypage/setrepbadge`, {rep_badge_id : myBadge.data.badge_id}, {withCredentials: true});
