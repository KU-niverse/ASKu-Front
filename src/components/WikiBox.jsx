...
const WikiBox = (props) => {
  ...
  const contentWithResponsiveImages = content.replace(
    /<img/g,
  ...
  return (
    <div className={styles.wikiContents}>
      <li onClick={toggleView} className={styles.wikiContentlist}>
        ...
        <div
          className={isZero ? `${styles.hidden}` : `${styles.wikiContentBtns}`}
        >
      ...
      <div className={isOpen ? `${styles.wikiText}` : `${styles.hidden}`}>
        <div
          dangerouslySetInnerHTML={{ __html: contentWithResponsiveImages }}
        />
      </div>
    </div>
  );
