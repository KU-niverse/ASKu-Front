...
useEffect(() => {
  const takeDebateContent = async () =>{
    try{
      const res = await axios.get( `https://localhost:8080/debate/view/${title}/${debateId}`, {withCredentials: true});
      if(res.status === 200){
...
const handleDebateSubmit = async (submitData) => {
  try {
    const res = await axios.post(`https://localhost:8080/debate/${title}/new/${debateId}`, submitData, {withCredentials: true});
    if(res.status === 200){
      setData(res.data);
