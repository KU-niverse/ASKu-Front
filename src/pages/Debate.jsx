...
useEffect(() => {
  const takeDebateContent = async () =>{
    try{
      const res = await axios.get( `${process.env.REACT_APP_HOST}/debate/view/${title}/${debateId}`, {withCredentials: true});
      if(res.status === 200){
...
const handleDebateSubmit = async (submitData) => {
  try {
    const res = await axios.post(`${process.env.REACT_APP_HOST}/debate/${title}/new/${debateId}`, submitData, {withCredentials: true});
    if(res.status === 200){
      setData(res.data);
