...
const MoreQuestion = () => {
  ...
  useEffect(() => {
    const takeQuestion = async () =>{
      try{
        const res = await axios.get( `https://localhost:8080/question/view/${flag}/${title}`, {withCredentials: true});
        if(res.status === 200){
  ...
  const handleQuestionSubmit = async (submitData) => {
    try {
      const res = await axios.post(`https://localhost:8080/question/new/${title}`, submitData, {withCredentials: true});
      if(res.status === 200){
        setData(res.data);
